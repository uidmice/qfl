...
class QLinear(nn.Module):
    ...
    def forward(self, input):
        ...
        if torch.isinf(act).any():
            raise ValueError('linear act: inf in QLinear forward')
        ...
        if torch.isnan(out).any():
            raise ValueError('linear out: nan in QLinear forward')
        if torch.isinf(out).any():
            raise ValueError('linear out: inf in QLinear forward')
...
class QConv2d(nn.Module):
    ...
    def forward(self, input):
        ...
        if torch.isinf(act).any():
            raise ValueError('conv act: inf in QLinear forward')

        out = F.conv2d(act, self.weight, stride=self.stride, padding=self.padding)
        out_s = combine_scale(act_s, self.weight_scale)
        
        if torch.isnan(out).any():
            raise ValueError('conv out: nan in QLinear forward')
        if torch.isinf(out).any():
            raise ValueError('conv out: inf in QLinear forward')
        return out, out_s
